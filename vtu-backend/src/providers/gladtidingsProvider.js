...
async function purchaseData({ network, plan_code, mobile_number }) {
  ...
  try {
    ...
    if (isSuccessResponse(data)) {
      ...
      console.log(`[gladtidingsProvider] Data purchase SUCCESS (HTTP ${response.status}) — transaction receipt:`, JSON.stringify(data ?? null));
...
async function getBalance() {
  try {
    ...
    throw new Error(`[gladtidings] getBalance: ${extractErrorMessage(error)}`);
  }
}
