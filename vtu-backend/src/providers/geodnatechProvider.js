...
async function verifyCableIUC({ iuc, identifier }) {
  ...
  if (!cablenameId) {
    throw new Error(
      `[geodnatech] verifyCableIUC: unrecognised cable provider "${identifier}". Re-sync cable plans from the active provider before verifying.`
    );
  }
  ...
  try {
    ...
    if (data && (data.invalid === true || (typeof data.invalid === 'string' && String(data.invalid).toLowerCase() === 'true'))) {
      const err = new Error(`[geodnatech] verifyCableIUC: ${extractProviderMessage(data, 'Invalid IUC/Smartcard')}`);
...
// ---------------------------------------------------------------------------
// Geodnatech DISCO list — disco_id ⇄ disco name (from Geodnatech docs)
// ---------------------------------------------------------------------------
// Documented Geodnatech electricity DISCOs. These numeric disco_id values are
...
//   (6)  6 = Port Harcourt Electric   (12) 12 = Aba Electric
const DISCO_LIST = [
  ...
  { disco_id: 12, name: 'Aba Electric' },
];

/**
 * Build electricity plans for Geodnatech.
 * The DISCO list comes from _fetchDiscoList(), which uses the documented
 * Geodnatech disco IDs/names (DISCO_LIST, e.g. 1=Ikeja Electric, 11=Yola
 * Electric, 12=Aba Electric) and supplements them with the live /disco/ endpoint.
...
async function getElectricityPlans() {
  try {
    ...
    // documented Geodnatech disco IDs/names and supplements with the live /disco/ endpoint.
...
// the Geodnatech docs (1=Ikeja … 11=Yola, 12=Aba). The live /disco/ endpoint is
// still consulted so any DISCO Geodnatech adds later is merged in, but a
// documented disco_id/name is never overridden. The app may send a disco as a
// numeric id, a slug ('ikeja-electric'), or a name ('Ikeja Electric'); resolve
// it to the numeric disco_id required by the provider's /validatemeter/ and
...
async function _fetchDiscoList() {
  ...
  try {
    ...
    console.warn(`[geodnatechProvider] /disco/ fetch failed — using documented DISCO list. ${err.message}`);
  }
...
async function _resolveDiscoId(value) {
  ...
  if (isNumeric) {
    ...
    if (!found) {
      throw new Error(
        `[geodnatech] disco_id ${num} was not found in the active provider's electricity list. ` +
        `Re-sync electricity plans from the active provider (geodnatech) before purchasing.`
      );
    }
    return num;
  }
...
async function purchaseElectricity({ meter, plan, amount, phone, type = 'prepaid', customerName, customerAddress }) {
  ...
  try {
    // Geodnatech's /billpayment/ is a DRF "Bill Payment" endpoint — the same
    // platform family as Gladtidings. It expects `disco_name` carrying the
    // NUMERIC disco_id (the pk that /validatemeter/ uses in its `disco_id`
    // query param), plus camelCase `MeterType` and `Customer_Phone`. Sending
    // `disco_id` and lowercase `meter_type` in the body makes the provider
    // reject the request with an opaque empty HTTP 500.
    const disco_id   = await _resolveDiscoId(plan);
